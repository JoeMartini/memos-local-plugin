...
function suppressIsolatedMathSkill(ranked) {
    const skillSlots = ranked.filter((r) => r.candidate.refKind === "skill");
    if (skillSlots.length !== 1)
        return [...ranked];
    const onlySkill = skillSlots[0]?.candidate;
    if (onlySkill && shouldKeepIsolatedMathSkill(onlySkill))
        return [...ranked];
...
function shouldKeepIsolatedMathSkill(skill) {
...
function renderWholePacket(snippets, reason, opts) {
    ...
    const guidanceBlock = renderDecisionGuidance(opts.decisionGuidance, standaloneMathFinalAnswer);
 ...
 * not a foreign block.
 */
function renderDecisionGuidance(g, standaloneMathFinalAnswer = false) {
    if (!g)
        return null;
    if (g.preference.length === 0 && g.antiPattern.length === 0)
        return null;
    const lines = standaloneMathFinalAnswer
        ? [
...
const MATH_HEADER_BY_REASON = {
    turn_start: "# Retrieved prior problem-solving memories\n\n" +
    ...
    skill_invoke: "# Invoked method memory\n\n" +
...
function footerFor(skillMode, snippets, standaloneMathFinalAnswer = false) {
    if (standaloneMathFinalAnswer) {
        return [
