...
async function runAll(deps, ctx, opts, plan) {
    ...
    try {
        ...
        // channel to be armed.
        const haveKeywordChannel = !!compiled.ftsMatch || (compiled.patternTerms?.length ?? 0) > 0;
        ...
        const tier2Promise = wantTier2 && !noUsableChannel
            ? runTier2({ repos: deps.repos, config: deps.config, now: deps.now }, {
                ...
                excludeSessionId: ctx.reason === "turn_start" && sessionId && !deps.config.lightweightMemory
                    ? sessionId
                    : undefined,
            })
        ...
        const rawCandidateCount = tier1.length +
            tier2.traces.length +
        ...
        const mechanicalRanked = ctx.reason !== "decision_repair" &&
            ...
            ? ranked.ranked.filter((candidate) => bypassesKeywordConfirmation(candidate) || hasKeywordChannel(candidate))
            : ranked.ranked;
        ...
        const filterResult = opts.skipLlmFilter
            ...
            : await llmFilterCandidates({ query: queryText, ranked: mechanicalRanked, episodeId }, {
                llm: deps.llm ?? null,
                log,
                config: deps.config,
            });
...
function isStandaloneMathFinalAnswerContext(ctx) {
...
function hasKeywordChannel(candidate) {
    return (candidate.candidate.channels ?? []).some((channel) => channel.channel === "fts" ||
