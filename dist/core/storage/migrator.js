 ...
 * Idempotent.
 */
export function runMigrations(db, dir = defaultMigrationsDir()) {
...
function ensureSkillRepairOriginColumns(db) {
    if (!tableExists(db, "skills"))
        return;
    ensureColumn(db, "skills", "repair_origin", "INTEGER NOT NULL DEFAULT 0 CHECK (repair_origin IN (0,1))");
    ensureColumn(db, "skills", "strict_trial", "INTEGER NOT NULL DEFAULT 0 CHECK (strict_trial IN (0,1))");
}
function ensureEmbeddingRetryLeaseColumns(db) {
    const columns = new Set(db.prepare(`PRAGMA table_info(embedding_retry_queue)`)
        .all()
        .map((row) => row.name));
...
function ensureSkillUsageColumns(db) {
    ...
    if (!table)
        return;
    const columns = new Set(db.prepare(`PRAGMA table_info(skills)`)
        .all()
        .map((row) => row.name));
 ...
 */
export function runMigrationsForPath(openFn, dir) {
    const db = openFn();
    try {
