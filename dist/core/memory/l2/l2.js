...
export async function runL2(input, deps) {
     ...
     */
    const inductionEvidenceByPolicy = new Map();
    {
        const t0 = Date.now();
        ...
        for (const bucket of ready) {
            ...
            if (dup) {
                ...
                const evidence = inductionEvidenceByPolicy.get(dup.id) ?? new Set();
...
function policyNearDuplicateScore(a, b) {
    ...
    const boundary = optionalFieldSimilarity(a.boundary, b.boundary);
    const verification = optionalFieldSimilarity(a.verification, b.verification);
    const optionalFieldsCompatible = !boundaryPolarityConflicts(a.boundary, b.boundary) &&
        (boundary == null || boundary >= POLICY_NEAR_DUP_BOUNDARY_MIN) &&
        (verification == null || verification >= POLICY_NEAR_DUP_BOUNDARY_MIN);
    const contentFieldMatches = title >= POLICY_NEAR_DUP_TITLE_MIN ||
...
function boundaryPolarityConflicts(a, b) {
    ...
    if (!aa || !bb)
        return false;
    return ((Boolean(aa.positive) && Boolean(bb.negative) && textSimilarity(aa.positive, bb.negative) >= 0.45) ||
