...
export function createPipeline(deps) {
    ...
    async function startLightweightEpisode(sessionId, userText, meta, turnTs) {
     ...
     */
    async function openEpisodeIfNeeded(sessionId, userText, meta, agent) {
     ...
     */
    function sweepStalePausedEpisodes(nowMs) {
        if (nowMs - lastPausedSweepAt < PAUSED_SWEEP_THROTTLE_MS)
            return;
        ...
        try {
            ...
            for (const ep of rows) {
                ...
                if (topicState !== "paused" && topicState !== "interrupted")
                    continue;
                ...
                if (ageMs < PAUSED_AUTO_FINALIZE_MS)
                    continue;
                ...
                try {
                    session.sessionManager.finalizeEpisode(ep.id, {
                        patchMeta: {
                            topicState: "ended",
    ...
    async function retrieveTurnStart(input, plan) {
        const ctx = {
            reason: "turn_start",
            agent: input.agent,
