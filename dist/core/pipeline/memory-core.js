...
import { getHostLlmBridge, registerHostLlmBridge, } from "../llm/host-bridge.js";
import { createPipeline } from "./orchestrator.js";
import { wrapRetrievalRepos } from "./retrieval-repos.js";
import { namespaceFromHints, namespaceMeta, normalizeNamespace, ownerFromNamespace, isVisibleTo, visibilityWhere, } from "../runtime/namespace.js";
import { createHubRuntime } from "../hub/runtime.js";
import { llmFilterCandidates } from "../retrieval/llm-filter.js";
import { isStandaloneMathFinalAnswerTask, mergeMathFinalAnswerProtocol, STANDALONE_MATH_FINAL_ANSWER_TASK_KIND, } from "../retrieval/math-task.js";
// ─── Public bootstrap helpers ───────────────────────────────────────────────
const FINAL_HUB_LLM_FILTER_TIMEOUT_MS = 3_000;
const IMPORT_WRITE_BATCH_SIZE = 500;
function initialEmbeddingDimensions(provider) {
 ...
 */
export async function bootstrapMemoryCore(options) {
    const result = await bootstrapMemoryCoreFull(options);
    return result.core;
}
export async function bootstrapMemoryCoreFull(options) {
    ...
    try {
        ...
        throw new MemosError("config_invalid", `migrations failed for ${home.dbFile}: ` +
    ...
    function recordSystemError(role, detail) {
        try {
    ...
    function recordSystemModelStatus(role, detail) {
        try {
            repos.apiLogs.insert({
    ...
    try {
        embedder = createEmbedder({
            ...
            onError: (d) => recordSystemError("embedding", d),
            onStatus: (d) => recordSystemModelStatus("embedding", d),
        });
    }
    catch (err) {
    ...
    try {
        llm = createLlmClient({
            ...config.llm,
            onError: (d) => recordSystemError("llm", d),
            onStatus: (d) => recordSystemModelStatus("llm", d),
        });
    }
    catch (err) {
    ...
    try {
        ...
        if (evolverModel && evolverProvider) {
            reflectLlm = createLlmClient({
                ...
                fallbackToHost: true,
                onError: (d) => recordSystemError("skillEvolver", d),
                onStatus: (d) => recordSystemModelStatus("skillEvolver", d),
            });
 ...
 */
export function createMemoryCore(handle, home, pkgVersion, options = {}) {
    ...
    const toolStepByEpisode = new Map();
    let hubRuntime = null;
    let hubRuntimeConfig = handle.config;
    const skillStartedAtByPolicy = new Map();
    const skillRunDurationBySkill = new Map();
    const l2StartedAtByEpisode = new Map();
    ...
    // reflect/reward path.
    const STALE_EPISODE_TIMEOUT_MS = Math.max(handle.config.algorithm.session.mergeMaxGapMs * 2, 4 * 60 * 60 * 1000);
    let lastStaleScan = 0;
    let lastDirtyClosedScan = 0;
    async function autoFinalizeStaleTasks() {
        const nowMs = Date.now();
        if (nowMs - lastStaleScan < 30_000)
            return;
        lastStaleScan = nowMs;
        try {
    ...
    async function autoRescoreDirtyClosedEpisodes() {
        const nowMs = Date.now();
        if (nowMs - lastDirtyClosedScan < 30_000)
            return;
    ...
    async function searchHubMemoryHits(query, limit = 5) {
        if (!hubRuntimeConfig.hub.enabled || !hubRuntime || !query.trim())
            return [];
        try {
            const hits = await withTimeout(hubRuntime.searchMemories(query, limit), 1_500, "hub_search_timeout");
    ...
    function isStandaloneMathFinalAnswerTurn(turn) {
    ...
    async function init() {
        ...
        try {
            ...
            if (orphans.length > 0) {
                ...
                for (const ep of lightweight) {
                    handle.repos.episodes.close(ep.id, nowMs, ep.rTask ?? undefined);
                ...
                const stale = normalOrphans.filter((ep) => ep.rTask != null ||
        ...
        handle.buses.skill.onAny((evt) => {
            ...
            if (k === "skill.crystallization.started") {
                ...
                const durationMs = (skillId ? skillRunDurationBySkill.get(skillId) : undefined) ??
                ...
                writeApiLog(handle, log, "skill_evolve", {
                    ...
                    episodeId: episodeFromPolicy(handle, policyIdForSkill) ??
                ...
                const skillId = evt.skillId;
                const durationMs = durationSince(policyId ? skillStartedAtByPolicy.get(policyId) : undefined, eventTime(evt), policyId ? 1 : 0);
                if (policyId)
                ...
                writeApiLog(handle, log, "skill_generate", {
                    ...
                    policyId,
                    skillId,
                    episodeId: episodeFromPolicy(handle, policyId) ??
                        episodeFromSkill(handle, skillId),
        ...
        handle.buses.l3.onAny((evt) => {
            ...
            if (k === "l3.abstraction.started") {
                ...
                writeApiLog(handle, log, "world_model_generate", {
                    ...
                    title: evt.title,
                    episodeId: episodeFromWorldModel(handle, evt.worldModelId) ??
                        l3TriggerEpisodeId,
    ...
    function episodeRewardIsDirty(ep) {
        ...
        if (meta.lightweightMemory === true)
            return false;
        if (meta.rewardDirty && typeof meta.rewardDirty === "object")
            return true;
        const reward = meta.reward;
        if (reward && typeof reward === "object" && reward.skipped === true) {
            return false;
        }
        if (ep.rTask == null &&
            (ep.traceIds?.length ?? 0) > 0 &&
            ...
            if (tr.toolCalls.length > 0) {
                tr.toolCalls.forEach((toolCall, idx) => {
                    turns.push({
                        ...
                        role: "tool",
                        content: typeof toolCall.output === "string"
                            ? toolCall.output
    ...
    async function health() {
        ...
        const skillEvolverInfo = resolveSkillEvolver(diskConfig ?? handle.config, 
        ...
        // in that case anyway.
        handle.reflectLlm ?? handle.llm, latestTraceTs());
        ...
        try {
            ...
            const hits = packet.snippets.map((snip) => {
                ...
                return {
                    tier,
                    refId: snip.refId,
                    refKind: snip.refKind === "preference" || snip.refKind === "anti-pattern"
                        ? "trace"
                        : snip.refKind,
                    score: snip.score ?? 0,
                    snippet: snip.body,
            ...
            try {
                ...
                const droppedIds = new Set((packet?.droppedByLlm ?? []).map((s) => s.refId));
                ...
                handle.repos.apiLogs.insert({
                    ...
                    output: ok
                        ? {
                            ...
                            stats: stats
                                ? withHubStats(retrievalStatsPayload(stats), hubCandidates.length, filtered.length, finalHubKept, finalFilterStats)
                                : undefined,
                        }
    ...
    async function submitFeedback(feedback) {
        ...
        const targetTrace = feedback.traceId
            ? handle.repos.traces.getById(feedback.traceId)
            : null;
        ...
        const episode = row.episodeId
            ? handle.repos.episodes.getById(row.episodeId)
            : null;
        const trace = row.traceId
            ? handle.repos.traces.getById(row.traceId)
            : null;
            ...
            toolCalls: [
                {
                    ...
                    errorCode: outcome.error || (normalizedOutcome !== "ok" && normalizedOutcome !== "unknown")
                        ? normalizedOutcome
                        : undefined,
                    startedAt: ts,
                    endedAt: ts,
    ...
    function moveUserTextToAnchoredDelegate(byId, delegateId, syntheticIds) {
        ...
        const source = syntheticIds
            .map((id) => byId.get(id))
            .find((row) => Boolean(row && row.turnId === delegate.turnId && row.userText.trim()));
        if (!source)
            return;
    ...
    async function searchMemory(query) {
        ...
        const sessionId = query.sessionId ??
            "adhoc-session-" + randomUUID().slice(0, 8);
        const ts = Date.now();
        const startedAt = Date.now();
        let ok = true;
        let candidates = [];
        ...
        try {
            ...
            if (query.topK) {
                const tierCaps = {};
                if (query.topK.tier1 !== undefined)
                    tierCaps[1] = query.topK.tier1;
                if (query.topK.tier2 !== undefined)
                    tierCaps[2] = query.topK.tier2;
                if (query.topK.tier3 !== undefined)
            ...
            retrievalStats = withHubStats(retrievalStatsPayload(s), hubCandidates.length, filtered.length, finalHubKept, finalFilterStats);
    ...
    async function countPolicies(input) {
        ...
        const needle = (input?.q ?? "").trim().toLowerCase();
        if (!needle) {
            return handle.repos.policies.list({ status: input?.status, limit: 100_000 }).filter((r) => (input?.includeAllNamespaces || visibleToCurrent(r)) && matchesNamespaceFilter(r, input)).length;
        }
        ...
        const rows = handle.repos.policies.list({ status: input?.status, limit: POLICY_SCAN_LIMIT }).filter((r) => (input?.includeAllNamespaces || visibleToCurrent(r)) && matchesNamespaceFilter(r, input));
    ...
    async function editPolicyGuidance(id, patch) {
        ...
        if (nextPref.length === current.preference.length &&
    ...
    async function countWorldModels(input) {
        ensureLive();
        const needle = (input?.q ?? "").trim().toLowerCase();
        const rows = handle.repos.worldModel.list({ limit: 100_000 }).filter((r) => (input?.includeAllNamespaces || visibleToCurrent(r)) && matchesNamespaceFilter(r, input));
        if (!needle)
            return rows.length;
    ...
    async function countEpisodes(input) {
        ensureLive();
        return handle.repos.episodes.list({ sessionId: input?.sessionId, limit: 100_000 }).filter((r) => (input?.includeAllNamespaces || visibleToCurrent(r)) &&
            matchesNamespaceFilter(r, input)).length;
    }
    async function listEpisodeRows(input) {
        ...
        const policiesByEpisode = new Map();
        ...
        const out = pagedRows.map((r) => {
            ...
            const episodeTraces = r.traceIds.length > 0
                ? handle.repos.traces.getManyByIds(r.traceIds)
                : [];
            let preview;
            const tagSet = new Set();
            if (firstTraceId) {
                const trace = episodeTraces.find((tr) => tr.id === firstTraceId) ??
                    handle.repos.traces.getById(firstTraceId);
                if (trace) {
            ...
            const meta = r.meta ?? {};
            if (!preview) {
                const fallback = typeof meta.initialUserText === "string"
            ...
            const reward = meta.reward && typeof meta.reward === "object"
                ? meta.reward
                : null;
            const rewardSkipped = reward?.skipped === true;
            const rewardReason = typeof reward?.reason === "string" && reward.reason.trim().length > 0
                ? reward.reason
                : null;
    ...
    async function listTraces(input) {
        ...
        if (input?.groupByTurn) {
            ...
            if (!needle) {
                ...
                turnKeys.forEach((k, i) => turnOrder.set(`${k.episodeId ?? "_"}:${k.turnId}`, i));
            ...
            const rows = handle.repos.traces.listByTurnKeys(orderedKeys).filter((r) => (input.includeAllNamespaces || visibleToCurrent(r)) && matchesNamespaceFilter(r, input));
    ...
    function matchesNamespaceFilter(row, input) {
        return ((!input?.ownerAgentKind || row.ownerAgentKind === input.ownerAgentKind) &&
    ...
    async function countSkills(input) {
        ensureLive();
        return handle.repos.skills.list({ status: input?.status, limit: 5_000 }).filter((r) => (input?.includeAllNamespaces || visibleToCurrent(r)) && matchesNamespaceFilter(r, input)).length;
    }
    ...
    function resolveSkillRowForGet(id, opts) {
        ...
        const candidates = handle.repos.skills.list({ limit: 5_000 }).filter((row) => {
            if (!opts?.includeAllNamespaces && !visibleToCurrent(row))
                return false;
            if (row.name === rawId || row.name === shortId)
                return true;
            if (rawId.includes(":"))
    ...
    async function metrics(input) {
        ...
        const policyToEpisodes = new Map();
        for (const p of policyRows) {
            policyToEpisodes.set(p.id, p.sourceEpisodeIds ?? []);
        }
        const evolvedEpisodes = new Set();
        for (const s of skillRows) {
            for (const pid of s.sourcePolicyIds ?? []) {
        ...
        const totalTurns = handle.repos.traces.countTurns({}, visibilityWhere(activeNamespace));
        return {
            total: totalTurns,
            writesToday,
    ...
    async function importBundle(bundle) {
        ...
        for (const batch of chunkArray(traces, IMPORT_WRITE_BATCH_SIZE)) {
            const result = handle.db.tx(() => {
                ...
                const existingIds = new Set(handle.repos.traces
                    .getManyByIds(valid.map((dto) => dto.id))
                    .map((row) => row.id));
                const addedByEpisode = new Map();
                for (const dto of valid) {
                    try {
                ...
                for (const [episodeId, ids] of addedByEpisode) {
                    ...
                    if (!episode)
                        continue;
                    handle.repos.episodes.appendTrace(episodeId, dedupeTraceIds([...episode.traceIds, ...ids]));
                }
    ...
    async function rebuildEmbeddings(input = {}) {
        ...
        if (batch.length > 0) {
            try {
                const vecs = await handle.embedder.embedMany(batch.map((slot) => ({ text: slot.sourceText || "(empty)", role: "document" })));
    ...
    function inferStoredEmbeddingDimension(slots) {
    ...
    function emptyEmbeddingStatsByKind() {
        const empty = () => ({
            totalSlots: 0,
            ready: 0,
            missing: 0,
    ...
    async function syncHubTraceShare(trace) {
        ...
        await runHubSync(trace.share?.scope === "hub"
            ? () => hubRuntime?.publishTrace(trace, row?.vecSummary ?? null)
            : () => hubRuntime?.unpublishTrace(trace.id), "trace", trace.id);
    }
    async function syncHubPolicyShare(policy) {
        await runHubSync(policy.share?.scope === "hub"
            ? () => hubRuntime?.publishPolicy(policy)
            : () => hubRuntime?.unpublishPolicy(policy.id), "policy", policy.id);
    }
    async function syncHubWorldModelShare(world) {
        await runHubSync(world.share?.scope === "hub"
            ? () => hubRuntime?.publishWorldModel(world)
            : () => hubRuntime?.unpublishWorldModel(world.id), "world_model", world.id);
    }
    async function syncHubSkillShare(skill) {
        await runHubSync(skill.share?.scope === "hub"
            ? () => hubRuntime?.publishSkill(skill)
            : () => hubRuntime?.unpublishSkill(skill.id), "skill", skill.id);
    }
...
function anchorSubagentRowsForDisplay(rows) {
    const delegateByToolCallId = new Map();
    const delegateByGoal = new Map();
...
// ─── Row → DTO mappers ───────────────────────────────────────────────────────
export function traceRowToDTO(row, episode) {
    return {
        id: row.id,
...
export function policyRowToDTO(row) {
    return {
        id: row.id,
        ownerAgentKind: row.ownerAgentKind,
        ownerProfileId: row.ownerProfileId,
...
export function worldModelRowToDTO(row) {
...
export function skillRowToDTO(row) {
...
export function inferTier(kind) {
    if (kind === "skill")
        return 1;
    if (kind === "world-model")
        return 3;
...
function resolveSkillEvolver(config, llm, fallbackTs) {
...
function writeApiLog(handle, log, toolName, input, output, durationMs, success) {
    try {
        handle.repos.apiLogs.insert({
            toolName,
            input,
...
function episodeFromPolicy(handle, policyId) {
    ...
    try {
        ...
        return (row.sourceEpisodeIds[row.sourceEpisodeIds.length - 1] ??
 ...
 */
export function deriveTurnCount(r, traces = []) {
    if (traces.length > 0) {
        return new Set(traces
            .map((trace) => trace.turnId)
            .filter((turnId) => Number.isFinite(turnId))).size;
    }
...
export function deriveSkillStatus(ep, relatedPolicies, skillsByPolicy, thresholds = {
...
function inferTurnRole(step) {
    ...
    if (u > 0 && (step.toolCalls?.length ?? 0) > 0)
        return "user";
