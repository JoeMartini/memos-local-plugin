...
import { computeEpisodeOutcome, DEFAULT_OUTCOME_THRESHOLDS, extractEpisodeVerifierPassed, } from "../episode/outcome.js";
import { backprop } from "./backprop.js";
import { scoreHuman } from "./human-scorer.js";
import { buildTaskSummary } from "./task-summary.js";
export function createRewardRunner(deps) {
    ...
    async function run(input) {
        ...
        const snapshot = deps.getEpisodeSnapshot?.(input.episodeId) ?? fallbackSnapshotFromRow(episode);
        ...
        try {
            const verifierPassed = extractEpisodeVerifierPassed(deps.feedbackRepo.getForEpisode(input.episodeId));
            const outcome = computeEpisodeOutcome(humanScore.rHuman, verifierPassed, outcomeCfg);
...
function decideSkipReason(snapshot, traces, cfg) {
