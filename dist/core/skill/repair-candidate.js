 ...
 */
export function isRepairCandidatePolicy(policy) {
    if ((policy.evidencePolarity ?? "positive") !== "negative")
        return false;
 ...
 */
export function deriveStrictTrial(policy) {
    const m = policy.verifierMeta;
    if (!m)
        return false;
 ...
 */
export function mintRepairCandidate(policy, deps) {
    if (!isRepairCandidatePolicy(policy))
        return null;
    ...
    const existingNames = new Set(deps.repos.skills.list({ limit: 500 }).map((s) => s.name));
    ...
    const row = {
        ...
        sourceWorldModelIds: [],
        evidenceAnchors: (policy.sourceTraceIds ?? []),
        vec: null,
        createdAt: now,
        updatedAt: now,
        version: 1,
...
function renderRepairGuide(policy, fix) {
    const lines = [];
    ...
    lines.push("Candidate fix distilled from a past failure on a similar task — applying it here both solves the task and confirms the fix.");
    lines.push("");
    ...
    if (antiPattern.length) {
        lines.push("**Avoid**");
        for (const a of antiPattern)
            lines.push(`- ${a}`);
        lines.push("");
    }
    return lines.join("\n").trim();
}
//# sourceMappingURL=repair-candidate.js.map