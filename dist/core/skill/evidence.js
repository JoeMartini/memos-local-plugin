...
export function gatherEvidence(policy, deps) {
    ...
    const excludedFailureCount = pool.filter((t) => epMeta.get(t.episodeId).outcome === "failure").length;
    const evidencePool = pool.filter((t) => epMeta.get(t.episodeId).outcome !== "failure");
    const deduped = dedupeTracesBySignature(evidencePool, (t, p) => scoreTrace(t, policy) - scoreTrace(p, policy));
 ...
 */
export function gatherIncrementalEvidence(policy, existingSkill, deps) {
    const anchors = new Set((existingSkill.evidenceAnchors ?? []));
    const pool = [];
...
export function gatherCounterExamples(policy, deps) {
    ...
    const epMeta = batchEpisodeMeta(pool, deps.repos);
    const failurePool = pool.filter((t) => epMeta.get(t.episodeId).outcome === "failure");
    const negativeInFailure = failurePool.filter((t) => Number.isFinite(t.value) && t.value < 0);
    ...
    const counterSource = negativeInFailure.length > 0
        ? negativeInFailure
...
function batchEpisodeMeta(pool, repos) {
    const epMeta = new Map();
    for (const epId of new Set(pool.map((t) => t.episodeId))) {
        const ep = repos.episodes.getById(epId);
        epMeta.set(epId, {
...
function annotateTrace(t, epMeta, charCap) {
...
function emptyEvidenceResult(episodeIds) {
    return {
        traces: [],
        episodeIds,
        medianValue: 0,
...
function dedupeTracesBySignature(traces, prefer) {
    const best = new Map();
    for (const t of traces) {
        const sig = traceIdentitySignature(t);
        const prev = best.get(sig);
        if (!prev || prefer(t, prev) < 0)
            best.set(sig, t);
    }
