...
const CORRECTIVE_CLAUSE_PATTERNS = [
...
function extractCorrectiveClause(text) {
    for (const re of CORRECTIVE_CLAUSE_PATTERNS) {
        const m = re.exec(text);
        const clause = m?.[1]?.trim();
        if (clause && substantive(clause))
            return clause;
    }
    return null;
}
 ...
 */
function constructiveFix(classified, llmProcedure) {
    const candidates = [
        llmProcedure,
    ...
    for (const c of candidates) {
        const s = c?.trim();
        if (s && substantive(s))
            return cleanLine(s, MAX_LINE_CHARS);
    }
    return null;
}
 ...
 */
export function objectiveOutcome(raw, rTask) {
