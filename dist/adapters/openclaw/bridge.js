 ...
 */
export function flattenMessages(input) {
    ...
    for (const raw of input) {
        ...
        if (TOOL_RESULT_ROLES.has(rawRole)) {
            ...
            const isError = typeof m.isError === "boolean" ? m.isError : undefined;
            const errorCode = typeof m.errorCode === "string" ? m.errorCode : undefined;
            out.push({
                role: "tool_result",
...
function pickToolCallId(block, message) {
    return firstString(block.id, block.toolCallId, block.tool_call_id, block.callId, block.call_id, block.toolUseId, block.tool_use_id, message?.toolCallId, message?.tool_call_id);
}
 ...
 */
const OPENCLAW_BOOT_SIGNATURES = [
 ...
 */
export function isOpenClawBootstrapMessage(raw) {
    const text = raw.trim();
    if (text.length === 0)
        return true;
    if (OPENCLAW_SENTINEL_REPLIES.has(text))
        return true;
    if (isOpenClawSubagentAnnouncementPrompt(text))
        return false;
    for (const sig of OPENCLAW_BOOT_SIGNATURES) {
        ...
        if (text.includes(sig)) {
            ...
            if (text.length > 400 && !looksLikeHumanTail(text))
                return true;
        }
    }
    return false;
}
function isOpenClawSubagentAnnouncementPrompt(raw) {
    const text = raw.trim();
 ...
 */
function looksLikeHumanTail(text) {
...
function stripOpenClawUserEnvelope(raw) {
    let text = raw;
    ...
    text = text.replace(/=== MemOS LONG-TERM MEMORY[\s\S]*?(?:MANDATORY[^\n]*\n?|(?=\n{2,}))/gi, "");
    text = text.replace(/\[MemOS Auto-Recall\][^\n]*\n(?:(?:\d+\.\s+\[(?:USER|ASSISTANT)[^\n]*\n?)*)/gi, "");
    text = text.replace(/## Memory system\n+No memories were automatically recalled[^\n]*(?:\n[^\n]*memos_search[^\n]*)*/gi, "");
    ...
    text = text
        ...
        const t = line.trim();
        if (!t)
            return true; // keep blank lines so paragraph breaks survive
        if (/^System(?:\s+\(untrusted\))?:/.test(t))
            return false;
        if (/^Current time:/i.test(t))
            return false;
        if (/^\[Untrusted\b/.test(t))
            return false;
        if (/^When reading HEARTBEAT\.md/i.test(t))
            return false;
        return true;
    })
        .join("\n");
 ...
 */
export function extractTurn(messages, now) {
    let lastUserIdx = -1;
    ...
    const toolCalls = [];
    const enqueuePendingCall = (key, stub) => {
    ...
    for (const m of tail) {
        if (m.role === "assistant") {
            if (m.content)
                pendingAssistant.push(m.content);
            continue;
        }
        if (m.role === "thinking") {
            if (m.content)
                pendingThinking.push(m.content);
            continue;
        }
...
function mergeToolCalls(captured, observed) {
...
function mergeToolCall(existing, observed) {
    return {
        ...observed,
        ...existing,
 ...
 */
export function bridgeSessionId(agentId, sessionKey) {
 ...
 * namespace.
 */
export function isEphemeralSessionKey(sessionKey) {
    if (!sessionKey)
        return false;
    return sessionKey.startsWith("temp:");
}
function isExplicitOneShotSessionKey(sessionKey) {
...
const BEFORE_PROMPT_SOFT_TIMEOUT_MS = Number.parseInt(process.env.MEMOS_BEFORE_PROMPT_SOFT_TIMEOUT_MS ?? "12000", 10);
const TOOL_FAILURE_REPAIR_HINT = "This tool has failed multiple times in a row. You may want to call `memos_search` for relevant past experience before deciding what to do next.";
 ...
 * extra context.
 */
export function renderContextBlock(packet, opts = {}) {
    if (!packet)
        return "";
...
function capContextBlock(block) {
...
function toolFailureStreakKey(toolName, event, ctx) {
    const run = ctx.runId ?? event.runId ?? ctx.sessionId ?? ctx.sessionKey ?? "global";
    return `${run}:${toolName}`;
}
function clearToolFailureStreaksForTurn(streaks, ctx) {
...
function toolResultPersistFailed(event) {
    if (event.error)
        return true;
    const msg = asRecord(event.message);
    if (msg) {
        if (msg.isError === true || msg.error === true)
            return true;
        if (typeof msg.error === "string" && msg.error.trim())
            return true;
        const details = asRecord(msg.details);
        if (details?.isError === true || details?.error === true)
            return true;
...
function appendFailureHintToToolResultMessage(message) {
    const msg = asRecord(message);
    if (!msg)
        return message;
    const content = msg.content;
    if (typeof content === "string") {
        if (content.includes(TOOL_FAILURE_REPAIR_HINT))
            return message;
    ...
    if (Array.isArray(content)) {
        ...
        if (idx >= 0) {
            ...
            if (text.includes(TOOL_FAILURE_REPAIR_HINT))
                return message;
            const next = [...content];
...
function appendFailureHint(content) {
    const trimmed = content.trimEnd();
    return `${trimmed}${trimmed ? "\n\n" : ""}${TOOL_FAILURE_REPAIR_HINT}`;
}
export function createOpenClawBridge(opts) {
    ...
    const latestEpisodeBySession = new Map();
    const episodeBindingByTurnKey = new Map();
    ...
    const toolFailureStreaks = new Map();
    const observedToolCallsBySession = new Map();
    let observedToolCallSeq = 0;
    const spawnedSubagents = new Map();
    const pendingSubagentSessions = new Set();
    function rememberObservedToolCall(sessionId, runId, tc) {
    ...
    function takeObservedToolCalls(sessionId, runId) {
        const list = observedToolCallsBySession.get(sessionId) ?? [];
        if (list.length === 0)
            return [];
        const matched = [];
        ...
        if (rest.length > 0)
            observedToolCallsBySession.set(sessionId, rest);
        else
    ...
    async function ensureSession(agentId, sessionKey, namespace) {
    ...
    function turnBindingKeys(sessionId, ctx, userText) {
        const keys = [];
        if (ctx.runId)
            keys.push(`${sessionId}\0run\0${ctx.runId}`);
        const text = userText?.trim();
        if (text)
            keys.push(`${sessionId}\0user\0${text}`);
        return keys;
    }
    function rememberEpisodeBinding(sessionId, episodeId, ctx, userText, seq = ++episodeBindingSeq) {
        const binding = {
            sessionId,
            episodeId,
            seq,
    ...
    function findEpisodeBinding(sessionId, ctx, userText) {
    ...
    function forgetEpisodeBinding(binding) {
        if (!binding)
            return;
        for (const key of binding.keys) {
    ...
    function forgetSessionBindings(sessionId) {
    ...
    function currentEpisodeId(sessionId) {
        return latestEpisodeBySession.get(sessionId)?.episodeId;
    }
    async function handleBeforePrompt(event, ctx) {
        const startedAt = now();
        try {
            ...
            const turnStartResult = await withSoftTimeout(turnStartPromise, BEFORE_PROMPT_SOFT_TIMEOUT_MS);
            ...
            const routedSessionId = (packet?.query.sessionId ?? sessionId);
            const routedEpisodeId = packet?.query.episodeId;
            if (routedEpisodeId) {
            ...
            opts.log.info(`memos.onTurnStart returned hits=${packet?.hits.length ?? 0} ` +
    ...
    async function handleAgentEnd(event, ctx) {
        ...
        try {
            ...
            const novel = cursor >= allMessages.length
                ? allMessages.slice()
            ...
            const toolCalls = mergeToolCalls(turn.toolCalls, takeObservedToolCalls(sessionId, ctx.runId));
    ...
    function handleBeforeToolCall(event, ctx) {
    ...
    async function handleAfterToolCall(event, ctx) {
        if (isEphemeralSessionKey(ctx.sessionKey))
            return;
        try {
            const sessionId = bridgeSessionId(ctx.agentId ?? "main", ctx.sessionKey ?? "default");
    ...
    function handleToolResultPersist(event, ctx) {
        if (isEphemeralSessionKey(ctx.sessionKey))
            return;
        ...
        if (nextCount < TOOL_FAILURE_HINT_THRESHOLD)
            return;
    ...
    async function handleSessionEnd(event, ctx) {
        if (isEphemeralSessionKey(ctx.sessionKey))
            return;
        try {
            const sessionId = bridgeSessionId(ctx.agentId ?? "main", ctx.sessionKey ?? "default");
    ...
    function handleSubagentSpawned(event, ctx) {
        const key = event.runId || event.childSessionKey || ctx.childSessionKey;
        if (!key)
            return;
        const parentAgentId = ctx.agentId ?? event.agentId ?? "main";
        const parentSessionKey = ctx.requesterSessionKey ?? ctx.sessionKey;
        ...
        if (parentSessionId)
            pendingSubagentSessions.add(parentSessionId);
    ...
    async function handleSubagentEnded(event, ctx) {
        try {
            const cached = (event.runId ? spawnedSubagents.get(event.runId) : undefined) ??
                spawnedSubagents.get(event.targetSessionKey);
            if (event.runId)
                spawnedSubagents.delete(event.runId);
