...
import { createOpenClawBridge } from "./bridge.js";
import { acquireOpenClawRuntimeLock, DuplicateOpenClawRuntimeError, } from "./runtime-lock.js";
...
function writeSharedRuntimeState(state) {
    const g = globalThis;
    if (state)
        g[SHARED_RUNTIME_KEY] = state;
    else
        delete g[SHARED_RUNTIME_KEY];
}
/**
...
function resolveViewerStaticRoot() {
...
async function createRuntime(api, runtimeLock) {
    ...
    try {
        ...
        const telemetry = new Telemetry(config.telemetry ?? {}, home.root, PLUGIN_VERSION, rootLogger.child({ channel: "core.telemetry" }), resolvePluginRoot());
        ...
        try {
            ...
            if (e?.code === "EADDRINUSE") {
                api.logger.error(`memos-local: viewer port :${OPENCLAW_VIEWER_PORT} is already in use — ` +
...
async function closeViewerAfterFailedBootstrap(viewer) {
    if (!viewer)
        return;
    try {
        await viewer.close();
...
function createSharedRuntimeState(api) {
    let runtimeLock;
    try {
        ...
        api.logger.error("memos-local: duplicate OpenClaw runtime blocked", {
            err: err instanceof Error ? err.message : String(err),
            code: duplicate ? err.code : err.code,
        });
        throw err;
    }
    const state = {
...
function register(api) {
    ...
    api.registerMemoryCapability?.({
        promptBuilder: ({ availableTools }) => {
            ...
            if (hasSearch) {
                lines.push("- `memos_search` — search prior traces, policies, world models, and skills when prior experience is likely relevant.");
            }
            if (hasEnv) {
                lines.push("- `memos_environment` — list / query accumulated environment knowledge " +
                    "(project layout, behavioural rules, constraints). Use before exploring an unfamiliar area.");
            }
            if (hasGet || hasTimeline) {
                lines.push("- `memos_get` / `memos_timeline` — fetch full bodies + episode timelines.");
            }
            if (hasSkillList) {
                lines.push("- `memos_skill_list` — list MemOS-crystallized skills learned from prior runs.");
            }
            if (hasSkillGet) {
                lines.push("- `memos_skill_get` — load the full invocation guide for a MemOS skill.");
            }
            lines.push("- Prefer concrete recalled memory when it is present. For standalone math tasks with no recalled memory, solve directly instead of searching an empty store.", "");
            return lines;
        },
    });
...
const plugin = {
    id: PLUGIN_ID,
    name: "MemOS Local",
    description: "Reflect2Evolve memory plugin — L1 traces, L2 policies, L3 world models, " +
        "skill crystallization, three-tier retrieval, decision repair.",
    register,
};
export default plugin;
/** Re-export the plain factory for tests / custom hosts. */
export function defineMemosLocalOpenClawPlugin(overrides) {
    return {
