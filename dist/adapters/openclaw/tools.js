...
const MemoryGetParams = Type.Object({
    ...
    kind: Type.Optional(Type.Union([Type.Literal("trace"), Type.Literal("policy"), Type.Literal("world_model")], { default: "trace" })),
});
...
const EnvironmentQueryParams = Type.Object({
    query: Type.Optional(Type.String({
        description: "Free-text keyword to narrow down (optional; omit to list all environments).",
    })),
...
function textToolResult(details, text) {
    return {
        ...details,
...
async function resolveCore(opts) {
...
// ─── Registration ──────────────────────────────────────────────────────────
export function registerOpenClawTools(api, opts) {
    const bodyCap = opts.maxBodyChars ?? DEFAULT_BODY_CAP;
    // ── memos_search ──
    api.registerTool((ctx) => ({
        name: "memos_search",
        label: "Memory Search",
        description: "Search MemOS memory (local traces + policies + world models + skills, plus connected Team Hub memories). " +
    ...
    api.registerTool((ctx) => ({
        name: "memos_get",
        label: "Memory Get",
        description: 'Fetch the full body of a memory item by id. `kind` can be "trace" (default), ' +
        ...
        async execute(_toolCallId, params) {
            ...
            if (kind === "trace") {
                const trace = await core.getTrace(params.id, namespaceFromCtx(ctx));
                if (!trace) {
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Return the ordered traces inside a single episode. Useful for reconstructing " +
            "conversation flow and debugging.",
        parameters: MemoryTimelineParams,
        async execute(_toolCallId, params) {
            const core = await resolveCore(opts);
            const traces = await core.timeline({ episodeId: params.episodeId, namespace: namespaceFromCtx(ctx) });
    ...
    api.registerTool((ctx) => ({
        ...
        label: "Skill List",
        description: "List callable skills the agent can invoke. Filter by status (candidate | active | archived).",
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Return the agent's accumulated environment knowledge (L3 world models) — " +
    ...
    api.registerTool((ctx) => ({
        ...
        async execute(toolCallId, params) {
            const core = await resolveCore(opts);
            const skill = await core.getSkill(params.id, {
                recordUse: true,
                recordTrial: true,
...
function topKParams(params, maxResults) {
    if (params.tier1topK === undefined &&
